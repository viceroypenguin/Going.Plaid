...
{
	/// <summary>
	/// <para>– The proposed transfer has received the end user's consent and has been approved for processing by Plaid. The <c>decision_rationale</c> field is set if Plaid was unable to fetch the account information. You may proceed with the transfer, but further review is recommended (i.e., use Link in update to re-authenticate your user when <c>decision_rationale.code</c> is <c>LOGIN_REQUIRED</c>). Refer to the <c>code</c> field in the <c>decision_rationale</c> object for details.</para>
	/// </summary>
