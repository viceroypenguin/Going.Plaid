...
public sealed partial class PlaidClient
{
	...
	/// <para>Calling <c>/item/remove</c> is a recommended best practice when offboarding users or if a user chooses to disconnect an account linked via Plaid. For subscription products, such as Transactions, Liabilities, and Investments, calling <c>/item/remove</c> is required to end subscription billing for the Item. For money movement products such as Auth and Transfer, if the Item is at an institution that uses Tokenized Account Numbers (TANs), such as Chase or PNC, calling <c>/item/remove</c> will invalidate the TAN, and subsequent ACH transfer attempts using that TAN will be returned.</para>
